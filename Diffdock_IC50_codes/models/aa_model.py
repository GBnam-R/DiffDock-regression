...
class AAModel(torch.nn.Module):
    ...
    def AAModel_embedding(self, data):
        """
        AAModel에서 embedding을 추출하는 목적
        """
        ...
        node_attr_ = torch.cat(node_attr, edge_index[:, :s3], edge_attr_, edge_sh[:s3], edge_weight=edge_weight[:s3])
        ...
        #         node_attr_conv = self.conv_layers[l](node_attr, edge_index[:, :s3], edge_attr_, edge_sh[:s3], edge_weight=edge_weight[:s3])
        
        return node_attr, edge_index, edge_attr, edge_sh, edge_weight, edge_attr_
