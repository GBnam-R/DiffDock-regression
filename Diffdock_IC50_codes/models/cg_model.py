...
class CGModel(torch.nn.Module):
    ...
    # def CGModel_embedding(self, data):
    #     """
    #     Extract embeddings from CGModel in models/cg_model.py
    #     copied forward method (in CGModel class) and return node_attr
    #     """
    ...
    #     node_attr = torch.cat([lig_node_attr, rec_node_attr], dim=0).to(device)
    ...
    #                             torch.flip(lr_edge_index, dims=[0])], dim=1).to(device)
    #     edge_attr = torch.cat([lig_edge_attr, lr_edge_attr, rec_edge_attr, lr_edge_attr], dim=0).to(device)
    #     edge_sh = torch.cat([lig_edge_sh, lr_edge_sh, rec_edge_sh, rev_lr_edge_sh], dim=0).to(device)
    #     edge_weight = torch.cat([lig_edge_weight, lr_edge_weight, rec_edge_weight, lr_edge_weight],
    #                             dim=0).to(device) if torch.is_tensor(lig_edge_weight) else torch.ones((len(edge_index[0]), 1),
    ...
    def forward(self, data):
        ...
        if self.no_torsion or data['ligand'].edge_mask.sum() == 0: return tr_pred, rot_pred, torch.empty(0, device=self.device), sidechain_pred, node_attr
        ...
        return tr_pred, rot_pred, tor_pred, sidechain_pred, node_attr
