...
def sampling(data_list, model, inference_steps, tr_schedule, rot_schedule, tor_schedule, device, t_to_sigma, model_args,
    ...
    with torch.no_grad():
        for batch_id, complex_graph_batch in enumerate(loader):
            ...
            for t_idx in range(inference_steps):
                ...
                _, _, _, _, node_attr = model(mod_complex_graph_batch)

                # node_attr를 의 마지막 embedding만 저장
                if t_idx == inference_steps-1:
                    final_embedding = node_attr
                    final_complex_graph = copy.deepcopy(mod_complex_graph_batch)
                    # print(type(final_complex_graph))
    ...
    return data_list, confidence, final_embedding, final_complex_graph
